...
class UpdateInvoiceRequest extends FormRequest
{
    ...
    {
        $currentStatus = $this->invoice->status;
        ...
        $statusOrder = ['draft', 'sent', 'paid', 'cancelled'];
        ...
        if ($newIndex < $currentIndex) {
            $validator->errors()->add(
                'status',
                "Cannot change status from {$currentStatus} to {$newStatus}. You can only progress forward."
            );
        }
        ...
        if ($currentStatus === 'draft' && $newStatus === 'paid') {
            $validator->errors()->add(
                'status',
                "Cannot change status from draft to paid directly. Please change to 'sent' first."
            );
        }
    }

    /**
     * Validate client ownership (for regular users only)
     */
